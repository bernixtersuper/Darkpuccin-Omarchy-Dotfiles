...
Item {
  ...
  // NOTE -- these three are `required property` in the built-in bar, and that
  // is exactly why a cloned bar never loads. shell.qml instantiates the
  // built-in one from an inline Component (shell.qml:227) where it can set
  // required properties up front, but a plugin bar goes through
  // `pluginBarLoader` (shell.qml:246), which only has a `source:` URL and
  // injects the properties afterwards in onLoaded/configureBar. A component
  // with unset required properties fails at creation, so the Loader lands in
  // Loader.Error, shell.failedBarId is set, and the shell falls back to
  // omarchy.bar and rewrites bar.id in shell.json. (Its own error branch
  // throws "ReferenceError: errorString is not defined", which is why the
  // fallback is silent about the reason.)
  //
  // Dropping `required` and defaulting them is enough: configureBar assigns
  // all three in onLoaded, and until then the bar renders off fallbackBarConfig
  // below, which is what it already does whenever barConfig is null.
  //
  // The omarchyPath default matches OMARCHY_PATH's value on this machine.
  ...
  // Set instead of barDragTarget when the pointer is over a section that holds
  // no widgets -- there is no slot to aim at, so the drop is described by the
  // section itself. See moduleDropAtScene / BarModel.emptyRegionDropTarget.
  ...
  function moduleDropAtScene(scenePoint, sourceSlot) {
    ...
    var candidates = []
    var regions = {
      left: { count: 0, min: Infinity, max: -Infinity },
      center: { count: 0, min: Infinity, max: -Infinity },
      right: { count: 0, min: Infinity, max: -Infinity }
    }
    ...
    for (var i = 0; i < moduleSlots.length; i++) {
      ...
      // Same measurement, collapsed per section, for the empty-section pass
      // below. The dragged slot is already excluded, so pulling the last
      // widget out of a section makes that section read as empty and it stays
      // reachable on the way back.
      var bucket = regions[slot.region]
      if (bucket) {
        var start = root.vertical ? slotPoint.y : slotPoint.x
        var extent = root.vertical ? slot.height : slot.width
        bucket.count += 1
        bucket.min = Math.min(bucket.min, start)
        bucket.max = Math.max(bucket.max, start + extent)
      }
    }

    // An empty section owns no slot, so nearestDropTarget can never name it.
    // Ask first whether the pointer is over one; only fall through to the
    // slot-relative targeting when it is not.
    var emptyDrop = root.emptyRegionDropAtScene(regions, scenePoint, sourceWindow)
    if (emptyDrop) return emptyDrop

    return BarModel.nearestDropTarget(candidates, scenePoint, root.vertical)
  }

  // Maps a scene point onto a section that currently holds no widgets.
  // Returns the same shape nearestDropTarget does, minus `slot`, plus the run
  // of bar the section was matched on so the drop marker can be drawn there.
  function emptyRegionDropAtScene(regions, scenePoint, sourceWindow) {
    var content = sourceWindow ? sourceWindow.contentItem : null
    ...
    var barStart = root.vertical ? origin.y : origin.x
    var barEnd = barStart + (root.vertical ? content.height : content.width)

    var hit = BarModel.emptyRegionDropTarget(regions, axis, barStart, barEnd)
    if (!hit) return null

    return { slot: null, after: false, region: hit.region, sliceStart: hit.start, sliceEnd: hit.end }
  }

  // Drop marker for an empty section. Modules land at the section's own anchor
  // rather than under the cursor -- left packs against the left edge, right
  // against the right -- so the marker is drawn at that anchor, not where the
  // pointer happens to be.
  function emptyRegionMarkerRect(drop) {
    if (!drop || !drop.region) return null

    var window = root.barDragWindow
    ...
    var axis = drop.region === "left"
      ? drop.sliceStart
      : (drop.region === "right" ? drop.sliceEnd : (drop.sliceStart + drop.sliceEnd) / 2)

    var thickness = Style.spacing.xs
    var scenePoint = root.vertical
      ? { x: origin.x, y: axis - thickness / 2 }
      : { x: axis - thickness / 2, y: origin.y }
    var screenPoint = root.barDragScreenPoint(scenePoint)

    if (root.vertical) {
      return { x: screenPoint.x, y: screenPoint.y, width: content.width, height: thickness }
    }

    return { x: screenPoint.x, y: screenPoint.y, width: thickness, height: content.height }
  }
    ...
    MouseArea {
      ...
      onPositionChanged: function(mouse) {
        ...
        if (dragging) {
          ...
          root.barDragTargetGeometry = !drop
            ? null
            : (drop.slot ? root.dropMarkerRect(drop.slot, drop.after) : root.emptyRegionMarkerRect(drop))
        }
      }
      ...
      onReleased: function(mouse) {
        ...
        if (wasDragging && targetSlot) {
          ...
          // Empty section: no neighbour to sit before, so the module is
          // appended -- an empty list makes that the only slot anyway.
