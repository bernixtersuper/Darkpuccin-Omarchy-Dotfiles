...
DISPLAY_NUM=1
STEP=5
STATE_FILE="/tmp/waybar_brightness2.tmp"
# ---------------------
...
current=$(tail -1 "$STATE_FILE" | grep -oE '[0-9]+')
if [ -z "$current" ]; then
    current=100 # Default fallback value
fi
...
case "$1" in
    ...
    "up")
        ...
        pkill -RTMIN+4 waybar # Can add whatever RTMIN+n you please, as long as it's an integer. Make sure the signals match between the config and script.
        ;;
    "down")
